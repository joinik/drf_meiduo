...
class EmailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'email']
        # user模型类继承的，email 可以为空，所以这里必须 修改为必传，才能更新
        extra_kwargs = {
            'email': {
                'required': True
            }
    ...
    def update(self, instance, validated_data):
        ...
        if not re.match(r'^[a-z0-9][\w\.\-]*@[a-z0-9\-]+(\.[a-z]{2,5}){1,2}$', instance.email):
            raise serializers.ValidationError('参数email有误')
